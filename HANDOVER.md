...
今回、Supabaseプロジェクトが消えて全データを失いました。原因は
**すべてのサービスが1人の個人アカウントにぶら下がっていた**ことです。
以下を必ず守ってください。

1. **各サービスのOwner/Adminは必ず2名以上。** 1名だとその人が
   離脱・退職・アカウント停止になった時点でプロジェクトが死にます。
2. **請求情報（クレジットカード）は事業主体のものを登録する。**
   個人カードのままだと、その人が抜けたとき支払いが止まり、
   Supabaseの無料枠プロジェクトは**一定期間後に自動削除**されます。
   今回の消滅もこのパターンの可能性が高いです。
...
| Supabase Organization | （記入） | （記入） | （記入） |
| Stripe | 熊田さん | （記入） | 熊田さん |
| ドメイン（取得する場合） | （記入） | — | （記入） |
